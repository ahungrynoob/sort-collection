...
fn main() {
    ...
    sync_lib::bubble_sort(arr1, "bubble_nearly_random_sort");
    sync_lib::selection_sort(arr2, "selection_nearly_random_sort");
    sync_lib::insertion_sort(arr3, "insertion_nearly_random_sort");
    sync_lib::heap_sort(arr4, "heap_nearly_random_sort");
    sync_lib::merge_sort(arr5, "merge_nearly_random_sort");
    sync_lib::quick_sort(arr6, "quick_nearly_random_sort");

    let elapsed_ms = start.elapsed().whole_milliseconds();
    println!("duration: {}", elapsed_ms);
}
